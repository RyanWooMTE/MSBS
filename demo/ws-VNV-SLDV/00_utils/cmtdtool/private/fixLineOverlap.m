        ...
        for m=n+1:length(lh)
            ...
            for m1=1:size(mPoints,1)-1
                ...
                elseif ovl == 2 % x-direction line overrap
                    if m1==1
                        if size(mPoints,1)>2 && ishandle(lh(m)) && get(lh(m),'LineParent') ~= -1
                            delete_line(lh(m))
                            add_line(sys,mPoints(2:end,:));
                        elseif size(Points,1)>2 && ishandle(lh(n)) && get(lh(n),'LineParent') ~= -1
                            delete_line(lh(n))
                            add_line(sys,Points(2:end,:));
                        end
