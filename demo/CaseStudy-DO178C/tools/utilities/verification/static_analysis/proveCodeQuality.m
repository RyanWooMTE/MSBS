...
tmplDir = fullfile(matlabroot, 'toolbox', 'polyspace', 'psrptgen', 'templates');
...
%psprjCfg.ChecksAssumption.ScalarOverflowsBehavior = 'wrap-around';
...
% Generate the report.
tmplFile = fullfile(tmplDir, 'Developer.rpt');
rptFile = fullfile(psprjDir, [modelName, '_Code_Prover_Report.pdf']);
rptCmd = ['"', rptgenExe, '"', ...
    ' -template "', tmplFile, '"', ...
    ' -format PDF', ...
    ' -output-name "', rptFile, '"', ...
...
system(rptCmd);
docDir = fullfile(psprjDir, [modelName, '_Code_Prover_Report_pdf_files']);
if exist(docDir, 'dir')
    rmdir(docDir, 's');
end
open(rptFile);
